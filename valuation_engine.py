...
class FinMindValuationEngine:
    ...
    def calc_yield_percentile_bounds(self, stock_id):
        """
        用 TaiwanStockPER 資料集裡的 dividend_yield 欄位(近5年歷史殖利率序列)，
        自動算出便宜/合理/目標殖利率門檻，取代人工目測設定的 target_yields。

        邏輯：殖利率越高代表股價相對股利越便宜，所以用歷史殖利率的第80百分位
        (歷史上少見的高殖利率)當「便宜」門檻，第50百分位(歷史中位數)當「合理」，
        第20百分位(歷史上少見的低殖利率)當「目標/該停利」門檻——跟 calc_pe_valuation
        用「PE第20百分位=便宜」的方向剛好相反，因為殖利率跟股價成反比。

        回傳 (便宜殖利率%, 合理殖利率%, 目標殖利率%)，抓不到資料時回傳 (0.0, 0.0, 0.0)。
        呼叫端應該只在 config 沒有手動指定 target_yields 時才使用本函式的結果，
        讓手動設定的門檻可以覆寫自動計算(例如你對某檔股票有特別的判斷依據時)。
        """
        df = self._fetch_data("TaiwanStockPER", stock_id, years_back=5)
        if df.empty or "dividend_yield" not in df.columns:
            return 0.0, 0.0, 0.0
        valid_yield = df[df["dividend_yield"] > 0]["dividend_yield"]
        if valid_yield.empty:
            return 0.0, 0.0, 0.0
        y_cheap = round(np.percentile(valid_yield, 80), 2)
        y_fair = round(np.percentile(valid_yield, 50), 2)
