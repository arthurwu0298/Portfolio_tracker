...
class TaiwanMarketTracker:
    ...
    def fetch_market_data(self):
        print("📥 抓取 TWSE / TPEx 最新報價與客觀指標(PE/PB/Yield)...")
        try:
    ...
    def plot_historical_trend(self):
        ...
        if not df_plot.empty:
            ...
            for column in df_plot.columns:
                plt.plot(df_plot.index, df_plot[column], label=column, linewidth=1.5)
                
            plt.title('投資組合核心持股走勢對比 (近半年累積報酬率 %)', fontsize=15, fontweight='bold', pad=15)
    ...
    def get_news_and_analysis(self, df):
        print("📰 正在抓取官方公告與媒體新聞，並啟動 AI 量化大腦...")
        ...
        if GEMINI_API_KEY:
            ...
            try:
                ...
                prompt = f"""
                你是一位頂尖的量化投資經理與財經分析交易員和專業財經主編。我會提供你投資組合目前的「現價、殖利率、PE、PB」等原始客觀數據，以及「官方公告」與「媒體新聞」，請根據上述資訊，提供最新最即時投資組合,AI相關台股,台股金融業重點分析並撰寫專屬我的盤後報告。
                
                【重要核心任務】
                請根據提供的「現價」、「本益比(PE)」、「淨值比(PB)」、「殖利率(%)」以及「指定估價法」，直接由你來估算每檔標的的「便宜價、合理價、昂貴(目標)價」，並決定「當前狀態」(例如：便宜(加碼)、合理(續抱)、偏高(留意)、達標(停利))。
                ...
                <div style='background-color: #f8f9fa; padding: 20px; border-radius: 8px; font-family: sans-serif; color: #333;'>
                  ...
                  <table style='width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 13px; text-align: center;'>
                    <tr style='background-color: #e9ecef;'>
                      ...
                      <th style='border: 1px solid #ccc; padding: 8px;'>最新參考價</th>
                  ...
                  <ul style='font-size: 13px; line-height: 1.8; padding-left: 20px;'>
                    <!-- 針對狀態為「達標(停利)」、「便宜(加碼)」或有重大新聞的標的，分板塊給出具體的操作建議與防守點位 -->
                  </ul>
                </div>
                
                【今日投資組合原始數據 (包含 PE/PB/Yield)】
    ...
    def send_email_notify(self, df, today_str):
        ...
        if self.fetch_errors:
            ...
            error_banner = f'''<div style="background-color:#fff3cd; border:1px solid #ffeeba; padding:10px 15px; border-radius:5px; margin-bottom:15px; font-size:12px;"><b>⚠️ 本次執行有資料抓取異常 (AI 將以備援數據進行推估)：</b><ul style="margin:6px 0 0 20px;">{error_items}</ul></div>'''
